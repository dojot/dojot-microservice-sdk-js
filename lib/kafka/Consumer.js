 ...
 */
const DEFAULT_CONSUMER_DISCONNECT_INTERVAL = 10000;
...
module.exports = class Consumer {
  ...
  constructor(config) {
    ...
    this.config['disconnect.interval.ms'] = (
      this.config['disconnect.interval.ms'] || DEFAULT_CONSUMER_DISCONNECT_INTERVAL
    );
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        const disconnectTimeout = setTimeout(() => {
          logger.error('Unable to disconnect the consume due to timeout.');
  ...
  async invokeInterestedCallbacks(data) {
    try {
      ...
      logger.warn(`Error during handle message calling: ${error}`);
    } finally {
