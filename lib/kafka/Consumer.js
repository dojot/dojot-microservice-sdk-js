...
module.exports = class Consumer {
   ...
   * the message's topic
   *
   * If a processing callback throws an exception, the callback will re recalled
    ...
    try {
      ...
      this.topicRegExpArray.forEach(async (entry) => {
        if (entry.regExp.test(data.topic)) {
          ...
          let succeeded = false;
          do {
            logger.debug(`Message on topic: ${data.topic} . Calling callback: ${entry.id}. Attempt: ${retry + 1}`);
            try {
          ...
          if (!succeeded) {
            ...
            this.eventEmitter.emit('error.processing');
          }
        }
      });
      ...
      logger.warn(`Error during processing message: ${error}`);
      this.eventEmitter.emit('error.processing');
    } finally {
