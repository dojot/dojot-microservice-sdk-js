...
class Manager {
  ...
  constructor(config = {}) {
    ...
    this.areServicesBeingRemoved = false;
    ...
    this.config = {
      lightship: {
        ...
        // The default is process.exit(1), but it is not graceful
  ...
  updateState(service, status) {
    // We need to make sure the server is not shutting down, or else it can throw if a service is
    // being removed from the map and this function is called before its remotion
    if (!this.areServicesBeingRemoved) {
  ...
  removeService(service) {
    if (this.services.has(service)) {
      ...
      this.logger.warn(`Cleared "${service}" health checker`);
      this.services.delete(service);
    } else {
      this.logger.warn(`Health checker for "${service}" not found`);
    }
  }
   ...
   */
  removeAllServices() {
    this.areServicesBeingRemoved = true;
    if (this.services.size > 0) {
      this.logger.warn(`Removing ${this.services.size} health checkers...`);
      this.services.forEach(
        (serviceData, service) => this.removeService(service),
      );
      this.services.clear();
      this.logger.warn('... successfully removed all health checkers');
    } else {
      this.logger.warn('No health checkers to remove');
    }
    this.areServicesBeingRemoved = false;
  }
}
