...
class ServiceStateManager {
  ...
  constructor(services, config = {}) {
    ...
    const instanceConfig = {
      ...
      logger: { ...defaultLoggerConfig, ...config.logger },
      module: { ...defaultModuleConfig, ...config.module },
    };

    Logger.setTransport('console', instanceConfig.logger.console);
    Logger.setVerbose(instanceConfig.logger.verbose);
    this.logger = new Logger('microservice-sdk:service-state-manager');

    this.logger.info('Initializing the ServiceStateManager...');

    this.logger.debug(`Config: ${util.inspect(instanceConfig)}`);
    ...
    this.lightship = lightship.createLightship(instanceConfig.lightship);
    ...
    this.initWorker(
      ...
      instanceConfig.module.workerFile,
    );
