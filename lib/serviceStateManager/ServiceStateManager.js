...
class ServiceStateManager {
  ...
  constructor(context, config = {}) {
    ...
    const defaultModuleConfig = {
      workerFile: 'healthcheck/Worker.js',
    };

    const lightshipConfig = { ...defaultLightshipConfig, ...config.lightshipConfig };
    const loggerConfig = { ...defaultLoggerConfig, ...config.logger };
    const moduleConfig = { ...defaultModuleConfig, ...config.module };

    Logger.setTransport('console', loggerConfig.console);
    Logger.setVerbose(loggerConfig.verbose);
    this.logger = new Logger('microservice-sdk:service-state-manager');

    this.logger.info(lightshipConfig);
    this.logger.info(loggerConfig);
    this.logger.info(moduleConfig);
    ...
    this.initWorker(path.join(path.dirname(require.main.filename), moduleConfig.workerFile));
